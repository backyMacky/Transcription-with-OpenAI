import sys
from PyQt5.QtWidgets import QApplication, QWidget, QPushButton, QVBoxLayout, QLabel, QHBoxLayout, QComboBox, QCheckBox, QFileDialog
...
class TranscriptionWorker(QThread):
    ...
    def __init__(self, max_record_time):
        super().__init__()
        self.max_record_time = max_record_time

    def run(self):
        transcribed_text = transcribe_audio(max_record_time=self.max_record_time)
...
class OverlayWindow(QWidget):
    ...
    def setup_window_properties(self):
        ...
        self.setGeometry(100, 100, 600, 650)  # Increased height to accommodate new controls
    ...
    def setup_recording_options(self, layout):
        ...
        self.max_time_dropdown.addItems(["5 seconds", "10 seconds", "30 seconds", "1 minute", "2 minutes", "5 minutes", "Unlimited"])
    ...
    def start_transcribing(self):
        ...
        self.transcription_worker = TranscriptionWorker(max_time)
    ...
    def on_transcription_done(self, transcribed_text):
        ...
        current_text = self.summary_label.text()  # Get current text
        new_text = f"Transcribed Text: {transcribed_text}\n"
    
        # Append the new transcribed text to the existing text in summary_label
        self.summary_label.setText(f"{current_text}\n{new_text}")
    
        if self.continuous_toggle.isChecked():
            self.start_transcribing()  # Start next transcription immediately
    ...
    def upload_prompt(self):
        ...
        if file_path:
            try:
                ...
                self.prompt_label.setText(f"Current Prompt: Custom ({os.path.basename(file_path)})")
        ...
        try:
            response = openai.ChatCompletion.create(
                ...
                messages=[
                    ...
                    {"role": "user", "content": f"Rewrite the following text in a {tone} tone:\n\n{self.transcribed_text}"}
                ]
            )
            self.rewritten_text = response['choices'][0]['message']['content']
            paste_text(self.rewritten_text)
            self.summary_label.setText(f"Rewritten Text ({tone}): {self.rewritten_text}")
        except Exception as e:
...
def transcribe_audio(silence_threshold=500, silence_duration=2, max_record_time=None):
    ...
    filename = f"output_{generate_random_string()}.wav"  # Generate filename with random characters
    p = pyaudio.PyAudio()
    ...
    while (data := stream.read(chunk)):
        frames.append(data)
        silent_chunks = silent_chunks + 1 if audioop.rms(data, 2) < silence_threshold else 0
        if silent_chunks > silence_limit or (max_chunks and len(frames) >= max_chunks): break
    ...
    try:
        ...
        with open(filename, "rb") as audio_file:  # Ensure the file is properly closed after reading
        ...
        # Ensure the file is properly closed before attempting to delete
